        ...
        <div id="uploadPreview" style="display:none;"></div>
         ...
         <div id="content">
             <div class="breadcrumb-container">
                <ol class="breadcrumb filemanager b-t small">
                    ...
                    <li data-folder="/" data-index="0"><span class="arrow"></span>
                        <span> {{ trans('systems/media.library') }}</span>
                    </li>
            ...
            <div class="flex">
                <div id="left" class="col">
                    <ul id="files">
                        <li v-for="(file,index) in files.items">
                            <div class="file_link" v-bind:data-folder="file.name" v-bind:data-index="index" v-bind:title="file.name">
                                <div class="link_icon">
                                    ...
                                    <i v-if="file.type.includes('video')" class="icon icon-camrecorder"></i>
                                    <i v-if="file.type.includes('audio')" class="icon icon-music-tone"></i>
                                    <i v-if="file.type == 'folder'" class="icon icon-folder-alt"></i>
                                    <i v-if="file.type != 'folder' && !file.type.includes('image') && !file.type.includes('video') && !file.type.includes('audio')" class="icon icon-doc"></i>
                                </div>
                    ...
                    <div id="file_loader">
                        <p>{{ trans('systems/media.loading') }}</p>
                    </div>

                    <div id="no_files">
                        <h3 class="font-thin"><i class="icon-directions"></i> {{ trans('systems/media.no_files_here') }}</h3>
                    </div>
                </div>
                ...
                <div id="right" class="col w-xxl no-padder">
                    <div class="right_none_selected">
                        ...
                        <p> {{ trans('systems/media.nothing_selected') }}</p>
                    </div>
                    <div class="right_details">
                        <div class="detail_img" v-if="selected_file != undefined && selected_file.type != undefined">

                            <a v-bind:href="selected_file.path" target="_blank" v-if="selected_file.type.indexOf('image')  !== -1">
                                <img v-bind:src="selected_file.path"/>
                            </a>

                            <video v-if="selected_file.type.indexOf('video') !== -1" width="100%" height="auto" controls>
                                ...
                                {{ trans('systems/media.video_support') }}
                            </video>
                            ...
                            <div v-if="selected_file.type.indexOf('audio') !== -1">
                                ...
                                <audio controls class="w-full">
                                    ...
                                    {{ trans('systems/media.audio_support') }}
                                </audio>
                            </div>

                            <i v-if="selected_file.type == 'folder'" class="icon-folder-alt"></i>

                            <i v-if="selected_file.type.indexOf('text') !== -1" class="icon-doc"></i>
                        </div>

                        <table class="table" v-bind:class="selected_file.type">
                            <tr>
                                <td width="50%"><small class="text-muted">{{ trans('systems/media.fileinfo.title') }}:</small></td>
                                <td>@{{ selected_file.name }}</td>
                            </tr>
                            <tr>
                                <td><small class="text-muted">{{ trans('systems/media.fileinfo.type') }}:</small></td>
                                <td>@{{selected_file.type}}</td>
                            </tr>
                            <tr v-if="selected_file.type != 'folder'">
                                <td><small class="text-muted">{{ trans('systems/media.fileinfo.size') }}:</small></td>
                                <td>@{{selected_file.size}}</td>
                            </tr>
                            <tr v-if="selected_file.type != 'folder'">
                                <td><small class="text-muted">{{ trans('systems/media.fileinfo.public_url') }}:</small></td>
                                <td><a v-bind:href="selected_file.path"
                                       target="_blank">{{ trans('systems/media.fileinfo.click_here') }}</a></td>
                            </tr>
                            <tr v-if="selected_file.type != 'folder'">
                                <td><small class="text-muted">{{ trans('systems/media.fileinfo.last_modified') }}:</small></td>
            ...
            <div class="nothingfound">
                ...
                <span>{{ trans('systems/media.no_files_here') }}</span>
            </div>
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-header">
                        ...
                        <h4 class="modal-title"><i class="icon-cursor-move"></i>{{ trans('systems/media.move_file_folder') }}</h4>
                    </div>

                    <div class="modal-body">
                        <h4>{{ trans('systems/media.destination_folder') }}</h4>
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-default" data-dismiss="modal">{{ trans('systems/media.cancel') }}</button>
                        <button type="button" class="btn btn-warning" id="move_btn">{{ trans('systems/media.move') }}</button>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-header">
                        ...
                        <h4 class="modal-title"><i class="fa fa-character"></i> {{ trans('systems/media.rename_file_folder') }}</h4>
                    </div>

                    <div class="modal-body">
                        <h4>{{ trans('systems/media.new_file_folder') }}</h4>
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-default" data-dismiss="modal">{{ trans('systems/media.cancel') }}</button>
                        <button type="button" class="btn btn-warning" id="rename_btn">{{ trans('systems/media.rename') }}</button>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-header">
                    ...
                    <h4 class="modal-title  padder-v">
                        ...
                        {{ trans('systems/media.add_new_folder') }}
                    </h4>
                </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-header">
                    ...
                    <h4 class="modal-title"><i class="icon-exclamation"></i>{{ trans('systems/media.delete') }}?</h4>
                </div>

                <div class="modal-body">
                    <h4>{{ trans('systems/media.sure_delete') }} '<span class="confirm_delete_name"></span>'</h4>
                    <h5 class="folder_warning"><i class="icon-exclamation"></i>
                    {{ trans('systems/media.delete_folder_question') }}</h5>
                </div>

                <div class="modal-footer">
                    <button type="button" class="btn btn-default" data-dismiss="modal">{{ trans('systems/media.cancel') }}</button>
                    <button type="button" class="btn btn-danger" id="confirm_delete">{{ trans('systems/media.yes_delete') }}
                    </button>
                </div>
            </div>
        </div>
